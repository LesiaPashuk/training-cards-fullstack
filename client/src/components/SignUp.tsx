...
export const SignUp = () => {
  ...
  const onSubmit = async (data: FormType) => {
    try {
      const response = await dispatch(asyncCreateUser(data)).unwrap();
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-[#5e041f] via-[#c92e2e] to-[#ffbaba] p-4">
      ...
      <form
        ...
        className="bg-white shadow-xl rounded-2xl p-8 max-w-md w-full flex flex-col justify-between"
      >
        ...
        <div className="mb-4">
          ...
          {errors.passwordFirst && (
            <p style={{ color: "red" }}>{errors.passwordFirst.message}</p>
          )}
        </div>
        <div>
          <input
          ...
          {errors.passwordSecond && (
            <p style={{ color: "red" }}>{errors.passwordSecond.message}</p>
          )}
        </div>
        <div>
          <button
          ...
          <button
            type="button"
            onClick={handleClear}
            className="mt-5 tracking-wide font-semibold bg-[#ffbaba] text-gray-100 w-full py-4 rounded-lg hover:bg-[#fcd8d8] transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none"
          >
